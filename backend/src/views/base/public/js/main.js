$(function() {
  ...
  if (window.ELASTIQUILL_ITEM_ID) {
    ...
    $('.elastiquill-share-select-text').prepend(makeButton('Contact', function(text) {
      var form = $('<form/>')
        .attr('method', 'POST')
        .attr('action', '/contact')
        .append($('<textarea name="contact-form-item-id">').val(window.ELASTIQUILL_ITEM_ID))
        .append($('<textarea name="contact-form-initial-value">').val(quoteText(text)))
    ...
    if ($('#comment-form').length) {
      $('.elastiquill-share-select-text').prepend(makeButton('Respond', function(text) {
