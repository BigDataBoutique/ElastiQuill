...
router.get('/', asyncHandler(async (req, res) => {
  res.redirect('https://www.linkedin.com/oauth/v2/authorization?'+ querystring.stringify({
    response_type: 'code',
...
router.get('/callback', passport.authenticate('jwt', { session: false }), asyncHandler(async (req, res) => {
  ...
  const profile = JSON.parse(await request('https://api.linkedin.com/v2/me?oauth2_access_token='+token));
