...
export function prepareComments(comments, allowComments, cb = null) {
  return comments.map(prepareComment).map(c => {
    c.allow_comments = allowComments;
    c.replies = prepareComments(c.replies || [], allowComments, cb);
    if (cb) {
      cb(c);
    }
    return c;
